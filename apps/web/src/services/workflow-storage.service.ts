...
const SEED_WORKFLOW: MatrixSchema = {
  ...
  description:
    "Automated credit decisioning workflow spanning intake, risk scoring, underwriting, and approval.",
  version: "1.0.0",
  columns: [
  ...
  rows: [
    {
      id: "row_applicant",
      label: "Applicant Data Interceptor",
      order: 0,
      type: "standard",
      isInterceptor: true,
    },
    {
      id: "row_bureau",
      label: "Bureau Credit Scorecard",
      order: 1,
      type: "standard",
    },
    {
      id: "row_sub_kyc",
      label: "KYC / AML Sub-Workflow",
      order: 2,
      type: "workflow",
      subWorkflowId: "wf_kyc_verification",
    },
    {
      id: "row_limit",
      label: "Credit Limit Calculator",
      order: 3,
      type: "standard",
    },
  ],
...
export class WorkflowStorageService {
  ...
  public static getAll(): MatrixSchema[] {
    ...
    try {
      ...
      console.error(
        "[WorkflowStorageService] Corrupted localStorage data for workflows:",
        err,
      );
      return [SEED_WORKFLOW];
    }
