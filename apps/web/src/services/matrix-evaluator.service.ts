...
function mapProtoResponseToResult(res: ExecuteMatrixResponse): MatrixExecutionResult {
  const stepRecords: StepEvaluationRecord[] = (res.stepRecords ?? []).map((step) => {
    const cellResults: CellResult[] = (step.cellResults ?? []).map((cell) => ({
      cellId: cell.cellId,
      rowId: cell.rowId,
      colId: cell.colId,
      action: mapCellAction(cell.action),
      status: (cell.status as 'success' | 'fail' | 'skipped') || 'success',
      mutatedPayload: cell.mutatedPayloadJson ? JSON.parse(cell.mutatedPayloadJson) : {},
    ...
    return {
      ...
      initialPayload: step.initialPayloadJson ? JSON.parse(step.initialPayloadJson) : {},
      finalPayload: step.finalPayloadJson ? JSON.parse(step.finalPayloadJson) : {},
      cellResults,
      emittedEvents: [],
    };
  });
