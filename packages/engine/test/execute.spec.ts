...
import { approvedInput, creditOriginationMatrix, declinedInput } from './fixtures/credit-origination';
...
describe('executeMatrixSync', () => {
  ...
  it('emits rule_evaluated with per-condition observability', () => {
    ...
    const scorecard = ruleEvents.find(
      (e) => e.type === 'rule_evaluated' && e.conditionResults.some((c) => c.key === 'creditScore'),
    );
    expect(scorecard).toBeDefined();
    if (scorecard?.type === 'rule_evaluated') {
      expect(scorecard.matched).toBe(false);
      const cs = scorecard.conditionResults.find((c) => c.key === 'creditScore')!;
  ...
  it('skips unsupported actions visibly', () => {
    ...
    expect(skipped.some((e) => e.type === 'action_skipped' && e.reason === 'unsupported' && e.actionType === 'trigger_sub_workflow')).toBe(true);
  });

  it('records payload mutations as deltas with before/after', () => {
    const log = runSample(approvedInput);
    const mutations = log.events.filter((e) => e.type === 'payload_mutated');
    const auth = mutations.find((e) => e.type === 'payload_mutated' && e.key === 'isAuthorized');
...
describe('execution semantics details', () => {
  ...
  it('all_matches applies every matching rule in order (later wins)', () => {
          ...
          actions: [
            {
              ...
              tableRuleConfig: {
                ...
                rules: [
                  ...
                  { conditions: { never: '== nope' }, mutations: { tier: 'GOLD' } },
                ],
              },
            },
          ],
    ...
    const log = executeMatrixSync(plan!, {}, { host: createDeterministicHost() });
    const result = toLegacyExecutionResult(log);
  ...
  it('disabled cells are skipped with a visible event', () => {
          ...
          actions: [
            {
              ...
              type: 'table_rule',
              enabled: true,
              tableRuleConfig: { rules: [{ conditions: {}, mutations: { hit: 'true' } }] },
            },
          ],
        },
      },
    ...
    const { plan } = compileMatrix(matrix);
    const log = executeMatrixSync(plan!, {}, { host: createDeterministicHost() });
    expect(log.events.some((e) => e.type === 'cell_skipped' && e.reason === 'disabled')).toBe(true);
  ...
  it('expression runtime errors mark the cell failed but execution continues', () => {
    const matrix: MatrixSchema = {
      ...
      cells: {
        ...
        'r1:c2': {
          ...
          colId: 'c2',
          action: 'table_rule',
          tableRuleConfig: { rules: [{ conditions: {}, mutations: { reached: 'true' } }] },
        },
      },
    };
    const { plan } = compileMatrix(matrix);
    ...
    const failedStep = result.eventLog.stepRecords.find((s) => s.cellResults[0]?.cellId === 'cell_boom')!;
  ...
  it('event_emitter actions emit', () => {
    ...
    const { plan } = compileMatrix(matrix);
    const log = executeMatrixSync(plan!, {}, { host: createDeterministicHost() });
    expect(log.events.some((e) => e.type === 'event_emitted' && e.eventName === 'PING')).toBe(true);
  });

  it("capture: 'none' omits payload values from events", () => {
    const { plan } = compileMatrix(creditOriginationMatrix);
    const log = executeMatrixSync(plan!, approvedInput, { host: createDeterministicHost(), capture: 'none' });
    const started = log.events[0]!;
    if (started.type === 'execution_started') expect(started.input).toBeUndefined();
