...
describe("compileMatrix", () => {
  ...
  it("reports bad expression syntax as an error and returns no plan", () => {
    ...
    expect(
      diagnostics.some(
        (d) => d.code === "BAD_EXPRESSION_SYNTAX" && d.severity === "error",
      ),
    ).toBe(true);
  });
  ...
  it("auto-migrates legacy raw-JS expressions to TEL with a warning", () => {
    const matrix = minimalMatrix({
      cells: {
        "r1:c1": {
          ...
          expressionConfig: {
            expression:
              "payload.riskResult === 'PASS_SCORECARD' ? 'APPROVED' : 'DECLINED'",
    ...
    expect(
      diagnostics.some(
        (d) =>
          d.code === "LEGACY_EXPRESSION_MIGRATED" && d.severity === "warning",
      ),
    ).toBe(true);
    ...
    if (action.kind === "expression") {
      expect(action.source).toBe(
        "riskResult == 'PASS_SCORECARD' ? 'APPROVED' : 'DECLINED'",
      );
    }
  });
  ...
  it("warns on unresolved inputs and output clashes", () => {
    ...
    expect(
      diagnostics.some(
        (d) => d.code === "UNRESOLVED_INPUT" && d.key === "neverProduced",
      ),
    ).toBe(true);
    expect(
      diagnostics.some(
        (d) => d.code === "OUTPUT_CLASH" && d.key === "givenInput",
      ),
    ).toBe(true);
    expect(
      diagnostics.some(
        (d) => d.code === "UNRESOLVED_INPUT" && d.key === "givenInput",
      ),
    ).toBe(false);
  });
          ...
          actions: [
            {
              ...
              outputs: ["derived"],
              tableRuleConfig: {
                rules: [{ conditions: {}, mutations: { derived: "1" } }],
              },
            },
          ],
        },
  ...
  it("produces a stable plan hash", () => {
    ...
    const b = compileMatrix(JSON.parse(JSON.stringify(creditOriginationMatrix)))
      .plan!.planHash;
    expect(a).toBe(b);
  });
});
