...
import { CompiledPlan, CompileResult, PlannedAction, PlannedCell, PlannedRule } from './plan';
...
function coercePayloadObject(obj: Record<string, unknown> | undefined): Record<string, Value> {
...
export function compileMatrix(matrix: MatrixSchema): CompileResult {
  ...
  const warn = (d: Omit<Diagnostic, 'severity'>) => diagnostics.push({ ...d, severity: 'warning' });
  const error = (d: Omit<Diagnostic, 'severity'>) => diagnostics.push({ ...d, severity: 'error' });
  ...
  for (let colIdx = 0; colIdx < columns.length; colIdx++) {
    ...
    for (let rowIdx = 0; rowIdx < rows.length; rowIdx++) {
      ...
      for (const act of sortByOrder(actions.map((a, i) => ({ ...a, order: a.order ?? i })))) {
        const enabled = act.enabled !== false;
        const coord = { rowId: row.id, colId: col.id, cellId, actionId: act.id };

        // Static dataflow validation (mirrors the editor's rules)
        for (const inputKey of act.inputs ?? []) {
          const resolved = workflowInputKeys.has(inputKey) || producedKeys.has(inputKey) || cellLocalOutputs.has(inputKey);
          if (!resolved) {
            warn({
        ...
        switch (act.type) {
          case 'table_rule': {
            ...
            for (let ruleIdx = 0; ruleIdx < rawRules.length; ruleIdx++) {
              ...
              const conditions = Object.entries(rule.conditions ?? {}).map(([key, cond]) => ({
                key,
                condition: compileCondition(cond == null ? '' : String(cond)),
              }));
              const mutations = Object.entries(rule.mutations ?? {}).map(([key, value]) => ({
                key,
              ...
              rules.push({
                ...
                emitEvent: rule.emitEvent
                  ? { eventName: rule.emitEvent.eventName, payload: coercePayloadObject(rule.emitEvent.payload) }
                  : undefined,
              });
            }
            planned.push({
          ...
          case 'expression': {
            ...
            try {
              ...
              planned.push({ kind: 'expression', id: act.id, enabled, source, ast, outputVariable });
              ...
              let migratedAst;
              try {
                migratedAst = migrated !== source ? parseExpression(migrated) : undefined;
              } catch {
              ...
              if (migratedAst) {
                ...
                planned.push({ kind: 'expression', id: act.id, enabled, source: migrated, ast: migratedAst, outputVariable });
                cellLocalOutputs.add(outputVariable);
                break;
              }
              const pos = e instanceof TelSyntaxError ? ` at position ${e.pos}` : '';
              error({
          ...
          case 'event_emitter': {
            planned.push({
              ...
              payload: coercePayloadObject(act.eventEmitterConfig?.eventPayload),
            });
            break;
          }
          ...
          default: {
            ...
            planned.push({ kind: 'unsupported', id: act.id, enabled, type: act.type });
            break;
          }
        }

        if (enabled) {
          for (const outputKey of act.outputs ?? []) cellLocalOutputs.add(outputKey);
        }
      }
      ...
      if (!disabled) {
        ...
        for (const a of planned) {
          if (a.kind === 'table_rule' && a.enabled) {
            for (const r of a.rules) for (const m of r.mutations) producedKeys.add(m.key);
          }
        }
      }

      cells.push({ cellId, rowId: row.id, colId: col.id, rowIdx, colIdx, disabled, actions: planned });
    }
  }
  ...
  const plan: CompiledPlan = {
    ...
    planHash: fnv1a(stableStringify({ id: matrix.id, columns, rows, cells: matrix.cells, inputs: matrix.inputs })),
