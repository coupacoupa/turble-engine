...
export function scopeFromObject(obj: Record<string, Value>): Scope {
  return {
    get: (name) =>
      Object.prototype.hasOwnProperty.call(obj, name) ? obj[name] : undefined,
  };
}
...
function compareOrdered(
  ...
  throw new TelRuntimeError(
    `Cannot compare ${typeName(a)} ${op} ${typeName(b)}`,
  );
}
...
function arithmetic(
  ...
  if (typeof a !== "number" || typeof b !== "number") {
    throw new TelRuntimeError(
      `Operator '${op}' requires numbers, got ${typeName(a)} and ${typeName(b)}`,
    );
  }
  switch (op) {
    case "+":
...
function containsOp(haystack: Value, needle: Value): boolean {
  if (typeof haystack === "string") {
    if (typeof needle !== "string") {
      throw new TelRuntimeError(
        `'contains' on a string requires a string operand, got ${typeName(needle)}`,
      );
    }
    return haystack.includes(needle);
  }
  if (Array.isArray(haystack))
    return haystack.some((el) => deepEquals(el, needle));
  throw new TelRuntimeError(
    `'contains' requires a string or array, got ${typeName(haystack)}`,
  );
}

function inOp(needle: Value, container: Value): boolean {
  if (Array.isArray(container))
    return container.some((el) => deepEquals(el, needle));
  if (container !== null && typeof container === "object") {
    if (typeof needle !== "string") {
      throw new TelRuntimeError(
        `'in' on an object requires a string key, got ${typeName(needle)}`,
      );
    }
  ...
  if (typeof container === "string") {
    if (typeof needle !== "string") {
      throw new TelRuntimeError(
        `'in' on a string requires a string operand, got ${typeName(needle)}`,
      );
    }
    return container.includes(needle);
  }
  throw new TelRuntimeError(
    `'in' requires an array, object, or string, got ${typeName(container)}`,
  );
}
...
export function evaluate(expr: Expr, scope: Scope): Value {
  switch (expr.kind) {
    ...
    case "index": {
      ...
      if (Array.isArray(obj)) {
        if (typeof idx !== "number" || !Number.isInteger(idx)) {
          throw new TelRuntimeError(
            `Array index must be an integer, got ${typeName(idx)}`,
          );
        }
        const v = obj[idx];
        return v === undefined ? null : v;
      ...
      if (obj !== null && typeof obj === "object") {
        if (typeof idx !== "string") {
          throw new TelRuntimeError(
            `Object index must be a string, got ${typeName(idx)}`,
          );
        }
        const v = obj[idx];
        return v === undefined ? null : v;
    ...
    case "unary": {
      ...
      if (typeof v !== "number") {
        throw new TelRuntimeError(
          `Unary '-' requires a number, got ${typeName(v)}`,
        );
      }
      return -v;
    }
    ...
    case "bin": {
      ...
      if (op === "||") {
        return truthy(evaluate(expr.left, scope))
          ? true
